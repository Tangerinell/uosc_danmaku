...
10. 自动记忆弹幕显示及自动匹配开关情况，播放视频时保持上次使用的两个开关状态
...
在 `controls`控件配置项中添加 `button:danmaku`的弹幕搜索按钮、`button:danmaku_auto`的自动匹配开关，以及 `cycle:toggle_on:show_danmaku@uosc_danmaku:on=toggle_on/off=toggle_off?弹幕开关`的弹幕显示开关。放置的位置就是实际会在在进度条上方的控件条中显示的位置，可以放在自己喜欢的位置。添加完控件的配置大概如下：

自动匹配开关和弹幕显示开关相互独立。自动匹配开关只控制联网的标题/源搜索，不影响同目录或 `save_danmaku_path` 中已保存 XML 的自动加载，也不影响手动搜索。通过 uosc 控件或快捷键切换自动匹配后，状态会写入 `history_path` 指定的历史文件，在切换视频及重启 mpv 后继续沿用；`auto_matching` 仅作为没有历史状态时的初始默认值。

```
controls=menu,gap,subtitles,<has_many_audio>audio,<has_many_video>video,<has_many_edition>editions,<stream>stream-quality,button:danmaku,button:danmaku_auto,cycle:toggle_on:show_danmaku@uosc_danmaku:on=toggle_on/off=toggle_off?弹幕开关,gap,space,speed,space,shuffle,loop-playlist,loop-file,gap,prev,items,next,gap,fullscreen
```
...
在 `controls`控件配置项中添加 `command:search:script-message open_search_danmaku_menu?搜索弹幕`的弹幕搜索按钮、`command:find_in_page:script-message toggle-auto-matching?弹幕自动匹配`的自动匹配开关，以及 `cycle:toggle_on:show_danmaku@uosc_danmaku:on=toggle_on/off=toggle_off?弹幕开关`的弹幕显示开关。放置的位置就是实际会在在进度条上方的控件条中显示的位置，可以放在自己喜欢的位置。添加完控件的配置大概如下：

```
controls=menu,gap,subtitles,<has_many_audio>audio,<has_many_video>video,<has_many_edition>editions,<stream>stream-quality,command:search:script-message open_search_danmaku_menu?搜索弹幕,command:find_in_page:script-message toggle-auto-matching?弹幕自动匹配,cycle:toggle_on:show_danmaku@uosc_danmaku:on=toggle_on/off=toggle_off?弹幕开关,gap,space,speed,space,shuffle,loop-playlist,loop-file,gap,prev,items,next,gap,fullscreen
```
...
对于坚定的键盘爱好者和不使用鼠标主义者，可以选择通过快捷键调用弹幕搜索、弹幕显示开关和自动匹配开关。

快捷键已经进行了默认绑定。默认情况下弹幕搜索功能绑定“Ctrl+d”；弹幕开关功能绑定“j”

弹幕搜索功能绑定的脚本消息为 `open_search_danmaku_menu`，弹幕显示开关功能绑定的脚本消息为 `show_danmaku_keyboard`，自动匹配开关功能绑定的脚本消息为 `toggle-auto-matching`。
...
Ctrl+Shift+d script-message toggle-auto-matching
```
...
auto_matching_keyboard_key=Ctrl+Shift+i
```
...
在视频播放时手动保存弹幕，保存格式为 `xml`（注：此功能将保存为视频同名弹幕，若目标文件夹下存在同名文件将不会执行该功能）。默认保存至视频所在文件夹，也可以通过 `save_danmaku_path` 和 `save_danmaku_local_mode` 保存到指定文件夹。
...
该选项控制是否进行联网自动搜索，不影响同目录或 `save_danmaku_path` 中已保存 XML 的自动加载、手动搜索，也不控制弹幕显示。通过 uosc 控件、快捷键或 `toggle-auto-matching` 切换后的状态会写入 `history_path` 指定的历史文件，因此切换视频和重启 mpv 后会保持上次状态；配置项只作为没有历史状态时的初始默认值。

自动匹配过程中，即使弹幕显示处于关闭状态，也会提示“自动搜索匹配弹幕中...”“弹幕加载中”“弹幕已准备好”或“自动匹配未找到弹幕”。匹配成功不会自动打开弹幕显示。
...
设置弹幕显示开关在没有历史记录时的初始状态。`yes` 表示打开，`no` 表示关闭。用户通过弹幕开关进行过切换后，状态会保存到 `history_path`，并优先于此配置项；删除或更换历史文件后才会重新使用默认值。

#### 使用方法

```
show_danmaku_default=yes
```
...
自动加载播放文件同目录下，或 `save_danmaku_path` 指定目录下与当前完整文件名对应的 xml 格式弹幕文件。文件名按保存时生成的完整名称精确查找，不使用标题候选或逐步缩短匹配。
...
> **⚠️NOTE！**
> 
> 当开启[autoload_local_danmaku选项](#autoload_local_danmaku)时，会按照保存路径配置使用当前完整文件名生成的 XML 路径进行精确查找，优先级高于一切其他自动加载弹幕功能。普通本地视频由 `save_danmaku_local_mode` 决定查找同目录或指定目录；ISO、DVD、BD 和 URL 直接查询 `save_danmaku_path`。如果不希望每次播放都加载之前保存的本地弹幕，则请关闭[autoload_local_danmaku选项](#autoload_local_danmaku)；或者在保存完弹幕之后转移弹幕文件至其他路径并关闭 `save_danmaku`选项。
> 
...
对于 ISO、DVD/BD 光盘路径和网络媒体，如需保存弹幕到指定文件夹，配置 `save_danmaku_path` 即可。目标目录（包括不存在的多级目录）会在首次保存时自动创建。普通本地视频默认保存到视频同目录，也可以通过 `save_danmaku_local_mode=specified_dir` 改为保存到指定目录。

保存 ISO 到指定文件夹时，文件名会包含 ISO 所在父目录名，用于减少不同目录下同名镜像的弹幕文件冲突。网络媒体保存到指定文件夹时使用媒体标题作为文件名。

`save_danmaku_local_mode` 用于选择普通本地视频的保存位置：

- `same_dir`：保存到视频同目录（默认值）
- `specified_dir`：保存到 `save_danmaku_path`；如果没有填写指定目录，则回退到视频同目录

ISO、DVD/BD 光盘路径和网络媒体不受 `save_danmaku_local_mode` 影响，统一使用 `save_danmaku_path`。

例如，将 ISO、DVD/BD 光盘路径和网络媒体的弹幕保存到 `~~/danmaku`：
...
```
tmdb_api_key=BASE64_ENCODED_TMDB_API_KEY
```
