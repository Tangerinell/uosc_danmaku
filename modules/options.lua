...
options = {
    ...
    -- 用户通过快捷键或 uosc 控件切换后的状态会持久化到 history_path，
    -- 因而跨文件和重启保持；此值只作为首次运行时的默认值。
    auto_matching = true,
    -- 弹幕显示开关的初始状态；历史文件中已有 show_danmaku 时以历史状态为准。
    show_danmaku_default = false,
    ...
    -- 播放结束时自动保存弹幕为xml文件
    save_danmaku = false,
    -- ISO/光盘/网络媒体的弹幕保存目录；普通本地视频始终保存到视频同目录
    save_danmaku_path = "",
    -- 普通本地视频的保存位置：same_dir / specified_dir；指定目录为空时回退到视频同目录
    save_danmaku_local_mode = "same_dir",
    ...
    -- 自动匹配总开关快捷键；留空表示不额外注册快捷键
    auto_matching_keyboard_key = "",
