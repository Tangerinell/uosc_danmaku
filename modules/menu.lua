...
-- 打开番剧数据匹配菜单
function get_animes(query, filter_note, extra_items)
...
-- 执行一次外部源 + 弹弹 Play 搜索，但不打开菜单。
local function search_anime_background_once(query, class, filter_note, callback)
    get_extra_search_items_async(query, class, function(extra_items)
        local encoded_query = url_encode(query)
        local all_server_metas = get_api_server_list(options.api_server, true)
        local server_metas = all_server_metas
        if filter_note and filter_note ~= "" then
            local matched = {}
            for _, meta in ipairs(all_server_metas) do
                if meta.note and meta.note == filter_note then
                    table.insert(matched, meta)
                end
            end
            if #matched > 0 then server_metas = matched end
        end
        local servers = {}
        for _, meta in ipairs(server_metas) do
            table.insert(servers, meta.url)
        end
        ...
        local function build_args(server)
            local url = server .. "/api/v2/search/anime?keyword=" .. encoded_query
            return make_danmaku_request_args("GET", url)
        end

        local function handle_response(server, err, out)
            if err then
                msg.debug(("background search failed for %s: %s"):format(server, tostring(err)))
                return
            end

            local data = utils.parse_json(out)
            if not data or not data.animes then return end

            for _, anime in ipairs(data.animes) do
                local key = anime.bangumiId or (anime.animeTitle and anime.animeTitle:gsub("%s+", " ") or nil)
                if key and not seen[key] then
                    seen[key] = true
                    server_items[server] = server_items[server] or {}
                    table.insert(server_items[server], {
                        title = anime.animeTitle,
                        hint = anime.typeDescription,
                        auto_type = anime.type,
                        value = {
                            "script-message-to", mp.get_script_name(), "search-episodes-event",
                            anime.animeTitle, anime.bangumiId, server,
                        },
                    })
                end
            end
        ...
        parallel_requests(servers, build_args, handle_response, function()
            for _, server in ipairs(server_order) do
                local list = server_items[server]
                if list then
...
-- 在搜索无结果时按标题自身的结构逐步缩短，避免为日期或片源格式维护特例。
-- callback 收到的 items 顺序与手动搜索菜单一致：外部源结果在前，随后是
-- 按 api_server 配置顺序合并的弹弹 Play 结果。
function search_anime_background(query, callback, class, filter_note, accept_results)
    local candidates = get_title_search_candidates(query)
    local index = 1
    ...
    local function try_next()
        ...
        search_anime_background_once(candidate, class, filter_note, function(items)
            local has_results = items and #items > 0
            local accepted = has_results and (not accept_results or accept_results(items, candidate))
            if accepted then
                callback(items, candidate)
                return
            end
            index = index + 1
            try_next()
...
function cache_anime_search_results(query, items)
    ...
    local menu_cmd = { "script-message-to", mp.get_script_name(), "search-anime-event" }
    if uosc_available then
        latest_menu_anime = utils.format_json({
            type = "menu_anime",
            title = "在此处输入视频名称",
...
-- 获取手动搜索输入框的默认文字：优先媒体标题，其次完整文件名，最后回退到路径文件名。
-- 这里只决定输入框预填内容，不参与自动匹配或搜索候选生成。
local function get_manual_search_default()
    local value = mp.get_property("media-title")
    if not value or value == "" then
        value = mp.get_property("filename/no-ext")
    end

    local from_path = false
    if (not value or value == "") then
        local path = mp.get_property("path")
        if path and path ~= "" then
            value = url_decode(path) or path
            from_path = true
        end
    end
    if not value or value == "" then return end

    value = normalize_title_whitespace(url_decode(value) or value)
    if value:find("[/\\\\]") then
        value = value:gsub("[/\\\\]+$", "")
...
function open_input_menu_get()
    mp.commandv('script-message-to', 'console', 'disable')
    local title = get_manual_search_default()
    input.get({
        prompt = '视频名称:',
        default_text = title,
...
function open_input_menu_uosc()
    ...
    local menu_props = {
        ...
        search_style = "palette",
        search_debounce = "submit",
        search_suggestion = search_title,
...
function toggle_danmaku_switch(value)
    ...
    mp.set_property_bool(DANMAKU_SWITCH_ON, enabled)
    update_danmaku_button(enabled)
end

-- 更新 UOSC 中的弹幕显示按钮。图标固定，仅让悬停提示反映当前状态。
function update_danmaku_button(enabled)
    mp.commandv(
        "script-message-to",
        "uosc",
        "set-button",
        ...
        utils.format_json({
            icon = "subtitles",
            tooltip = enabled == true and "弹幕：开" or "弹幕：关",
            command = "script-message show_danmaku_keyboard",
        })
    )
end

-- 更新由 UOSC controls 中 button:danmaku_auto 使用的自动匹配按钮状态。
-- 这个按钮只反映自动匹配总开关，不跟随弹幕显示开关变化。
-- 不使用 UOSC 的 active 高亮，避免开关状态过于醒目；状态通过悬浮提示显示。
function update_auto_matching_button(enabled)
    mp.commandv(
        "script-message-to",
        "uosc",
        "set-button",
        ...
        utils.format_json({
            icon = "find_in_page",
            tooltip = enabled == true and "弹幕自动匹配：开" or "弹幕自动匹配：关",
            command = "script-message toggle-auto-matching",
        })
    )
end
...
update_auto_matching_button(options.auto_matching ~= false)
update_danmaku_button(false)
...
mp.register_script_message("set", function(prop, value)
    ...
    set_danmaku_display_state(value == "on")
end)
...
mp.register_script_message("search-anime-event", function(query)
    ...
    -- 旧后缀仍可将外部影视搜索限制在单一分类；不带后缀时会搜索全部分类。
    local legacy_class = nil
    local name, class = search_name:match("^(.-)%s*|%s*(.-)%s*$")
    if name and ({ ds = true, dy = true, dm = true })[class:lower()] then
        search_name = name:gsub("%s+$", "")
        legacy_class = class
    end

    -- 外部影视分类异步加载完成后，再启动弹弹 Play 搜索，避免阻塞 mpv 菜单事件。
    search_generation = search_generation + 1
    local current_generation = search_generation

    -- 搜索期间保持 uosc 菜单可见，用菜单内的 spinner 表示等待状态。
    -- 不再通过 OSD 提示，也不关闭用户当前的搜索界面。
    if uosc_available then
        update_menu_uosc(
            "menu_anime",
            "在此处输入视频名称",
    ...
    search_anime_background(search_name, function(items)
        if current_generation ~= search_generation then return end
        show_anime_search_results(search_name, items)
    end, legacy_class, filter_note)
end)
...
mp.register_script_message('setup-danmaku-source', function(json)
    ...
    if event.type == 'activate' then
        ...
        if event.action == "delete" then
            ...
            mp.commandv("script-message-to", "uosc", "close-menu", "menu_source")
            open_add_menu()
            load_danmaku(true, nil, true)
        end

        if event.action == "block" then
            local source = DANMAKU.sources[event.value]
            source["blocked"] = true
            add_source_to_history(event.value, source)
            mp.commandv("script-message-to", "uosc", "close-menu", "menu_source")
            open_add_menu()
            load_danmaku(true, nil, true)
        end
